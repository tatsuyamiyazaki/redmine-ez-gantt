...
- 初期ログイン: `admin` / `admin`
- プラグイン配置: `./plugins/redmine_ez_gantt`

> [!WARNING]
> この Docker Compose 環境はローカル開発専用です。DB パスワード・`REDMINE_SECRET_KEY_BASE`・テストユーザーのパスワードが固定値でハードコードされているため、この構成のまま本番環境や外部公開環境で使用しないでください（ポートは `127.0.0.1` のみにバインドされています）。
